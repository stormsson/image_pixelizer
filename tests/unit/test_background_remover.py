...
class TestBackgroundRemover:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_rgb_to_rgba(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_rgba_to_rgba(self, mock_remove: Mock, sample_rgba_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_preserves_dimensions(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_creates_alpha_channel(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_rembg_integration(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_rembg_processing_error(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    def test_remove_background_with_different_sizes(self, mock_remove: Mock) -> None:
        ...
        mock_remove.return_value = mock_output_small
        ...
        mock_remove.return_value = mock_output_medium
    ...
    @patch("src.services.background_remover.remove")
    @patch("src.services.background_remover.new_session")
    def test_remove_background_with_sam_prompts(self, mock_new_session: Mock, mock_remove: Mock, sample_image_model: ImageModel) -> None:
        ...
        mock_new_session.assert_called_once_with("sam")
        ...
        assert call_kwargs["sam_prompt"] == prompts
        # Verify result
        assert isinstance(result, ImageModel)
        assert result.has_alpha is True
    ...
    @patch("src.services.background_remover.remove")
    @patch("src.services.background_remover.new_session")
    def test_remove_background_sam_model_initialization(self, mock_new_session: Mock, mock_remove: Mock, sample_image_model: ImageModel) -> None:
        ...
        assert mock_remove.call_args[1]["session"] == mock_session

    @patch("src.services.background_remover.remove")
    @patch("src.services.background_remover.new_session")
    def test_remove_background_sam_prompt_format_conversion(self, mock_new_session: Mock, mock_remove: Mock, sample_image_model: ImageModel) -> None:
    ...
    @patch("src.services.background_remover.remove")
    @patch("src.services.background_remover.new_session")
    def test_remove_background_sam_model_initialization_error(self, mock_new_session: Mock, mock_remove: Mock, sample_image_model: ImageModel) -> None:
        """Test that SAM model initialization errors raise BackgroundRemovalError."""
        from src.services.background_remover import BackgroundRemover

        # Mock new_session to raise an exception
        mock_new_session.side_effect = Exception("Model download failed")

        remover = BackgroundRemover(model="sam")
        with pytest.raises(BackgroundRemovalError) as exc_info:
            remover.remove_background(sample_image_model)

        # The error gets wrapped in the outer exception handler
        # Check that the technical message contains the initialization failure
        assert "Failed to initialize sam model" in str(exc_info.value) or "Failed to initialize sam model" in exc_info.value.technical_message

    @patch("src.services.background_remover.remove")
    def test_remove_background_sam_prompts_ignored_for_u2net(self, mock_remove: Mock, sample_image_model: ImageModel) -> None:
        ...
        # Verify remove was called without session or sam_prompt
        call_kwargs = mock_remove.call_args[1] if mock_remove.call_args[1] else {}
        assert "session" not in call_kwargs
        assert "sam_prompt" not in call_kwargs
